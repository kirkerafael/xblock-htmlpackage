...
@ddt
class HtmlPackageXBlockTests(unittest.TestCase):

    def make_one(self, **kw):
        """
        Creates a HtmlPackageXBlock for testing purpose.
        """
        field_data = DictFieldData(kw)
        block = HtmlPackageXBlock(mock.Mock(), field_data, mock.Mock())
    ...
    def test_fields_xblock(self):
        ...
        self.assertEqual(block.zip_file, None)
        self.assertEqual(block.zip_file_meta, {})
        self.assertEqual(block.icon_class, 'video')
        self.assertEqual(block.width, None)
        self.assertEqual(block.height, 450)

    def test_save_settings_htmlpackage(self):
        block = self.make_one()
